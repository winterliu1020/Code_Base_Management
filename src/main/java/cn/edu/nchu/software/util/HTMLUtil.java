...
public class HTMLUtil {
	...
	public HashMap<String, String> splitString(String string){
		String[] c = {"label bg-red","label bg-pink","label bg-purple","label bg-deep-purple","label bg-indigo"};
		int j = 0 ,k = 0;
		if(string == null || string.equals("")) {
			...
			while (string.indexOf(",") != -1) {
				...
				list.put(c[j++] + " " + k, string.substring(0,flag));
				k++;
				string = string.substring(flag + 1);
				if(j == c.length)
					j = 0;
			}
			list.put(c[j], string);
	...
	public String htmlToText(String str) {
		String text = html2text(str).replaceAll("\r|\n","");
		if(text.length() > 200)
